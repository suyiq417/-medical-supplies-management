...
const defaultConfig = reactive<DefaultConfigType>({
    ...
    showBottomValue: true, // 默认显示底部刻度
    axisVisible: true,    // 添加默认值
    labelVisible: true,   // 添加默认值
});
...
const mergeConfig = () => {
    mergedConfig.value = merge({}, defaultConfig, props.config || {});
};
                ...
                <div class="capsule-item" v-for="(capsule, index) in capsuleLength" :key="index">
                    <div class="capsule-item-column"
                        ...
                        <div v-if="mergedConfig.showValue" class="capsule-item-value">
                            {{ capsuleValue[index] }}{{ mergedConfig.unit }}
                        </div>
                    </div>
                </div>
            </div>
